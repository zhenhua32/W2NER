...
class Trainer(object):
    def __init__(self, model):
        ...
        self.scheduler = transformers.get_linear_schedule_with_warmup(
            self.optimizer, num_warmup_steps=config.warm_factor * updates_total, num_training_steps=updates_total
        )
                ...
                for ent_list, sentence in zip(decode_entities, sentence_batch):
                    ...
                    for ent in ent_list:
                        instance["entity"].append(
                            {"text": [sentence[x] for x in ent[0]], "type": config.vocab.id_to_label(ent[1])}
                        )
...
if __name__ == "__main__":
    ...
    logger.info("数据加载完成, 开始转换成 DataLoader")
